...
export type DetailedUnitPose = "idle" | "walkA" | "walkB" | "attack" | "hit" | "death";
...
function poseGeometry(
  ...
  for (let index = 0; index < positions.count; index += 1) {
    ...
    if (pose === "attack") {
      if (archetype === "archer") {
        if (group === 3) {
          [y, z] = rotateX(y, z, shoulderY, 0, 0.72);
          x -= 0.08;
          z -= 0.12;
        } else if (group === 4) {
          [y, z] = rotateX(y, z, shoulderY, 0, -0.82);
          z += 0.12;
        } else if (group === 6) z += 0.42;
      } else if (archetype === "knight") {
        if (group === 3) {
          [y, z] = rotateX(y, z, shoulderY, height * 0.12, -0.28);
          z += 0.22;
        } else if (group === 4) {
          [y, z] = rotateX(y, z, shoulderY, 0, -0.18);
        } else if (group === 10) {
          [y, z] = rotateX(y, z, height * 0.54, height * 0.25, -0.12);
        }
      } else if (archetype === "giant" && (group === 3 || group === 4)) {
        const side = group === 3 ? -1 : 1;
        [y, z] = rotateX(y, z, shoulderY, 0, -1.18 + side * 0.08);
        x -= side * 0.08;
        z += 0.22;
      } else if (group === 3 || group === 4) {
        [y, z] = rotateX(y, z, shoulderY, 0, group === 3 ? -1.08 : -0.24);
        if (group === 3) z += 0.15;
      }
    }
...
function guard(p: Palette): THREE.BufferGeometry[] {
  ...
  parts.push(
    ...
    part(5, new THREE.ConeGeometry(0.065, 0.19, 6), p.teamLight, 0, 1.58, 0),
    ...
    part(4, new THREE.CylinderGeometry(0.31, 0.31, 0.075, 12), p.team, 0.43, 0.83, 0.2, Math.PI * 0.5),
    part(4, new THREE.TorusGeometry(0.255, 0.035, 5, 16), p.goldDark, 0.43, 0.83, 0.24),
    part(4, new THREE.CylinderGeometry(0.065, 0.065, 0.07, 8), p.goldLight, 0.43, 0.83, 0.285, Math.PI * 0.5),
...
function knight(p: Palette): THREE.BufferGeometry[] {
  ...
  return [
    ...
    part(5, new THREE.SphereGeometry(0.22, 8, 5), p.skinDark, 0, 1.76, 0.05),
    ...
    part(3, new THREE.CylinderGeometry(0.035, 0.045, 2.05, 6), 0xd6d0bb, -0.47, 1.56, 0.72, Math.PI * 0.5),
    part(3, new THREE.ConeGeometry(0.09, 0.27, 6), p.metalLight, -0.47, 1.56, 1.87, Math.PI * 0.5),
    part(4, new THREE.CylinderGeometry(0.29, 0.29, 0.07, 10), p.team, 0.42, 1.4, 0.22, Math.PI * 0.5),
    part(4, new THREE.BoxGeometry(0.07, 0.38, 0.04), p.goldLight, 0.42, 1.4, 0.265),
  ];
}

function giant(p: Palette): THREE.BufferGeometry[] {
  const parts: THREE.BufferGeometry[] = [];
  addFeet(parts, p, 0.28, 0.62, 0.23);
  parts.push(
    part(0, new THREE.DodecahedronGeometry(0.7, 0), p.leatherLight, 0, 1.13, 0, 0, 0, 0, 1.1, 1.18, 0.82),
    ...
    part(0, new THREE.BoxGeometry(0.16, 0.56, 0.09), p.team, 0.43, 1.12, 0.58, 0, 0, -0.18),
    ...
    part(3, new THREE.DodecahedronGeometry(0.32, 0), p.skinDark, -0.83, 0.78, 0.18, 0, 0, 0, 1.18, 1, 1),
    part(4, new THREE.DodecahedronGeometry(0.32, 0), p.skinDark, 0.83, 0.78, 0.18, 0, 0, 0, 1.18, 1, 1),
...
function commander(p: Palette): THREE.BufferGeometry[] {
  ...
  parts.push(
    ...
    part(7, new THREE.ConeGeometry(0.48, 0.86, 8, 1, true, 0, Math.PI), p.team, 0, 0.85, -0.22, Math.PI * 0.5),
    ...
    part(5, new THREE.ConeGeometry(0.065, 0.25, 5), p.goldLight, 0.19, 1.78, -0.01),
    part(5, new THREE.SphereGeometry(0.045, 6, 4), p.team, 0, 1.63, 0.27),
